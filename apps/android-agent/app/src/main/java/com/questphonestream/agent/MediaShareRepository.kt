...
/** Small persisted repository for videos explicitly selected through SAF. */
class MediaShareRepository(context: Context?) {
    ...
    fun add(uri: Uri, mimeType: String?, displayName: String? = null, size: Long? = null): MediaItem {
        val name = displayName ?: uri.lastPathSegment?.substringAfterLast('/') ?: "video"
        val actualSize = size ?: -1L
        val id = newId()
        val item = MediaItem(id, name, mimeType ?: "video/*", actualSize, uri.toString(), detectSeekable(uri), true)
        items[id] = item
        persist()
        return item
    }
    ...
    private fun detectSeekable(uri: Uri): Boolean {
        // SAF providers may expose a pipe rather than a seekable descriptor. A real
        // file descriptor can be reopened and positioned by MediaHttpServer.
