...
{
    [Serializable] public sealed class SpatialCapabilityState { public bool available; public bool authorized; public bool active; }
    [Serializable] public sealed class SpatialCapabilityLimit { public string name; public string value; }
    [Serializable]
    public sealed class SpatialCapabilityDescriptor
    {
        public string name; public string version = "1.0"; public SpatialCapabilityState state;
        public string[] transports = Array.Empty<string>(); public string[] features = Array.Empty<string>();
        public SpatialCapabilityLimit[] limits = Array.Empty<SpatialCapabilityLimit>(); public string[] permissions = Array.Empty<string>();
    }
    ...
    public sealed class CapabilityRegistry
    {
        ...
        private CapabilityRegistry(IEnumerable<SpatialCapabilityDescriptor> capabilities)
        {
            ...
            {
                if (capability == null || string.IsNullOrWhiteSpace(capability.name)) throw new ArgumentException("Capability name is required");
                if (_capabilities.ContainsKey(capability.name)) throw new ArgumentException("Duplicate capability: " + capability.name);
        ...
        public bool UpdateState(string name, bool? available = null, bool? authorized = null, bool? active = null)
        {
            ...
            if (nextAvailable == capability.state.available && nextAuthorized == capability.state.authorized && nextActive == capability.state.active) return false;
            capability.state.available = nextAvailable; capability.state.authorized = nextAuthorized; capability.state.active = nextActive;
            Changed?.Invoke(All());
            return true;
        }
