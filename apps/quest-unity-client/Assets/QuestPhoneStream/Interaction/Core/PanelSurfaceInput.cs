...
namespace QuestPhoneStream.Interaction
{
    ...
    public sealed class AndroidScreenSurfaceInput : IPanelSurfaceInputHandler
    {
        ...
        public bool Process(PointerEvent pointer)
        {
            ...
            { Cancel(); return false; }
            if (pointer.phase == InteractionPhase.PressCancel) { Cancel(); return true; }
            if (pointer.phase == InteractionPhase.PressEnd)
            {
                if (!_active || pointer.source != _source) return false;
                if (_mapper.TryMapWorldPointToUv(pointer.worldPosition, pointer.worldNormal, out var end)) _last = end;
                ...
                else _mapper.SendClick(startPixels);
                return true;
            }
            ...
            if (pointer.phase == InteractionPhase.PressBegin)
            {
                ...
                _active = true; _source = pointer.source; _start = _last = uv; _began = Time.unscaledTime;
                return true;
            }
            if (_active && _source == pointer.source && pointer.phase == InteractionPhase.PressMove) { _last = uv; return true; }
            return false;
        }
        public void Cancel() { _active = false; _start = _last = default; }
    }
}
